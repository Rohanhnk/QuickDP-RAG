...
This project automates filling out a job application form from a candidate's resume. Instead of a person manually copying details from a resume into a form, the system:

- reads the resume and indexes it for semantic search (RAG),
- reads the blank job application form and figures out what information each field needs,
- turns each field into a question, and answers it by retrieving the relevant part of the resume,
- lets a human check the generated answers and correct anything wrong (by typing or by speaking), and
- produces the completed, filled-out form as the final output.

It's built as an **event-driven agentic workflow** (using LlamaIndex Workflows) rather than a single-shot script, so it can branch (correct vs. incorrect answers), loop (retry an answer after feedback), and process multiple form fields independently — the same pattern used for real-world document tasks like invoice enrichment, contract compliance checks, or insurance claims processing.
